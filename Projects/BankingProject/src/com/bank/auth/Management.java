...
//	regardless of login or registration
	...
	public void viewAccounts(User user)
	{
		...
		if (choice == 0)
			return;
		System.out.println("   • Account ID:\t" + user.getAccList().get(temp.get(choice-1)).getAccID());
		System.out.println("   • Balance:\t\t" + user.getAccList().get(temp.get(choice-1)).getBalance());
	}
	...
	public void deposit(User user)
	{
		...
		if (amount >= 0)
		{
			double balance = user.getAccList().get(temp.get(choice-1)).getBalance();
