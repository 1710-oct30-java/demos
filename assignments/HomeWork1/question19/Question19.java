...
public class Question19 {
	...
	public static void main(String[] args) {
		//Make an "ArrayList" from an int stream.  Need boxed to convert the IntStream into a Stream so I can collect as a List
		List<Integer> myList = (ArrayList<Integer>)IntStream.rangeClosed(1, 10).boxed().collect(Collectors.toList());
		//Convert the "ArrayList" to a stream. Remove all odds. Convert stream to an IntStream. Sum the IntStream
		...
		//Convert the "ArrayList" to a stream. Remove all evens. Convert the stream to an IntStream. sum the IntStream
	...
	{
		//For each number in the sieve.  If differentNumber % number: delete differentNumber.
