...
public class Question16 {
	public static void main(String[] args) {
		...
		String input = args[0];
		// Not 100% what is meant by number of characters so I'll count all the
		// individual characters
		Map<String, Integer> map = charCounter(input);
		// loop through the Hashmap and print out the contents
		for (String keys : map.keySet()) {
			System.out.println("There are " + map.get(keys) + " " + keys);
		}
		// Maybe you were actually just wanting the total number of character in the
		// string so here is the string length
		System.out.println("The string " + input + " has " + input.length() + " chars in it");

	}

	private static Map<String, Integer> charCounter(String s) {
