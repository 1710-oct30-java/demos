		...
		try {
			...
			for (int i = 0; i < reimbIdsToBeApproved.size(); i++) {
				PreparedStatement ps = conn.prepareStatement(
						"UPDATE ERS_REIMBURSEMENT SET reimb_status_id=2, reimb_resolver=?, reimb_resolved=? WHERE reimb_id=?");
	...
	public Boolean denyRequests(int uid, List<Integer> reimbIdsToBeDenied) {
		try {
			...
			for (int i = 0; i < reimbIdsToBeDenied.size(); i++) {
				PreparedStatement ps = conn.prepareStatement(
						"UPDATE ERS_REIMBURSEMENT SET reimb_status_id=3, reimb_resolver=?, reimb_resolved=? WHERE reimb_id=?");
